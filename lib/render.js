...
var srs_defaults = require('./srs_defaults');
var default_resolutions = srs_defaults.resolutions.split(',').map(parseFloat);
...
function calculateMetatile(options) {
    ...
    var resolutions = options.resolutions || default_resolutions;
    var map_minx = options.minx;
    var map_miny = options.miny;
    var map_maxx = options.maxx;
    var map_maxy = options.maxy;

    // Apply default spherical mercator map dimensions
    if(map_minx === undefined) map_minx = srs_defaults.minx;
    if(map_miny === undefined) map_miny = srs_defaults.miny;
    if(map_maxx === undefined) map_maxx = srs_defaults.maxx;
    if(map_maxy === undefined) map_maxy = srs_defaults.maxy;

    var units_per_tile = resolutions[options.z];
    ...
    meta_maxx = Math.min(meta_maxx, map_maxx);
    meta_miny = Math.max(meta_miny, map_miny);
    
    var meta_width = Math.round((meta_maxx - meta_minx) / units_per_px);
    var meta_height = Math.round((meta_maxy - meta_miny) / units_per_px);
    var meta_ntiles_x = Math.ceil(meta_width / options.tileSize); 
    var meta_ntiles_y = Math.ceil(meta_height / options.tileSize); 

    // Generate all tile coordinates that are within the metatile.
    var tiles = []; 
    for (var dx = 0; dx < meta_ntiles_x; dx++) {
        for (var dy = 0; dy < meta_ntiles_y; dy++) {


            var tile_minx = meta_minx + units_per_tile * dx;
            var tile_maxy = meta_maxy - units_per_tile * dy;
            var tile_maxx = tile_minx + units_per_tile;
            var tile_miny = tile_maxy - units_per_tile;
            console.log('units_per_tile', units_per_tile);
            console.log('meta_minx', meta_minx);
            console.log('meta_miny', meta_miny);
            console.log('meta_maxx', meta_maxx);
            console.log('meta_maxy', meta_maxy);
            console.log('tile_minx', tile_minx);
            console.log('tile_miny', tile_miny);
            console.log('tile_maxx', tile_maxx);
            console.log('tile_maxy', tile_maxy);
            ...
            tiles.push([ z, x + dx, y + dy, Math.round(tile_width), Math.round(tile_height)]);
        }
    }
...
MapnikSource.prototype._renderMetatile = function(options, callback) {
    ...
    return meta.tiles.map(function(tile) {
        return options.format + ',' + tile.slice(0, 3).join(',');
    });
};
