...
MapnikSource.prototype._normalizeURI = function(uri) {
    ...
    if (!uri.query.srs) {
        uri.query.srs = srs_defaults.srs;
        uri.query.minx = srs_defaults.minx;
        uri.query.miny = srs_defaults.miny;
        uri.query.maxx = srs_defaults.maxx;
        uri.query.maxy = srs_defaults.maxy;
        uri.query.resolutions = srs_defaults.resolutions;
    } else {
        if(typeof uri.query.minx !== "number" || typeof uri.query.minx !== "number" || typeof uri.query.minx !== "number" || typeof uri.query.minx !== "number") {
            throw new Error("Map bounds (minx, maxx, miny, maxy) must be provided if custom srs used");
        }
    }
    if(!uri.query.resolutions) {
        //by default, use resolutions that cause each zoom level to be twice the resolution of the previous zoom level
        var resolutions = [];
        for(var z = 0; z <= 30; z++){
            var map_width = (+uri.query.maxx) - (+uri.query.minx);
            var map_height = (+uri.query.maxy) - (+uri.query.miny);
            var max_res = Math.max(map_width, map_height);
            var units_per_tile = max_res / (1 << z);
            resolutions.push(units_per_tile);
        }
        uri.query.resolutions = resolutions.join(',');
    }
...
MapnikSource.prototype._open = function(uri) {
    ...
    uri.query.minx = +uri.query.minx;
    uri.query.maxx = +uri.query.maxx;
    uri.query.miny = +uri.query.miny;
    uri.query.maxy = +uri.query.maxy;
    uri.query.resolutions = uri.query.resolutions.split(',').map(parseFloat);
    this._uri = uri;
...
MapnikSource.prototype._createMetatileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        var keys = source._renderMetatile({
            ...
            resolutions: source._uri.query.resolutions,
            srs: source._uri.query.srs,
            minx: source._uri.query.minx,
            miny: source._uri.query.miny,
            maxx: source._uri.query.maxx,
            maxy: source._uri.query.maxx,
            format: coords[0],
            z: +coords[1],
            x: +coords[2],
            y: +coords[3]
